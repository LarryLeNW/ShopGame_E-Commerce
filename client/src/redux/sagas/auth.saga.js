...
function* loginSaga(action) {
    try {
        ...
        const { roleId, name } = result.data.user;
        if (result.data.errorCode !== 0) {
            yield put(
                loginFailure({ error: "Email hoặc mật khẩu không đúng" })
            );
            return;
        }
        yield callback(roleId, name);
        yield put(
            loginSuccess({
        ...
        const res = yield getDoc(doc(db, "userChats", result.data.user.id));
        const combinedId = "idadmin" + result.data.user.id;
        if (!res.exists()) {
            yield setDoc(doc(db, "userChats", result.data.user.id), {});
            yield setDoc(doc(db, "chats", combinedId), { messages: [] });
            if (result.data.user.roleId == "USER") {
                //create user chats
                ...
                yield updateDoc(doc(db, "userChats", result.data.user.id), {
                    [combinedId + ".userInfo"]: {
                        id: "idadmin",
                        name: "ADMIN",
...
function* getUserInfoSaga(action) {
    try {
        ...
        const result = yield axios.post(
            `http://localhost:6789/getUserByToken`,
            {
                token,
            }
        );
...
function* changeAvatarSaga(action) {
    try {
        ...
        let result = yield axios.put(
            `http://localhost:6789/editAvatarUser`,
            data
        );
...
function* loginOthersSaga(action) {
    try {
        ...
        let result = yield axios.post(
            "http://localhost:6789/loginOthers",
            data
        );
